...
void Set_RTC(u8 *command_rec, rtc_t *rtc)
{
	...
	rtc->day   =   0x05;
	*/
  rtc->hr  = command_rec[5] ; //  04:15:00 pm
  rtc->min = command_rec[6]  ;
  rtc->sec  = command_rec[7];
  rtc->am_pm= command_rec[8];
  rtc->dt = command_rec[9]; //5 March 2020
  ...
  rtc->day =command_rec[12] ;
	 ...
	 ds3231_set(rtc);  //  set date and time
	 
}

void display_RTC(rtc_t *rtc_read)
{
...
void Read_telemetry(u8 *telemetry)
{
	...
	I2C_Stop();
	telemetry[20]=getCRC(telemetry,20);
